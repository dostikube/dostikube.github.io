window.DOSTI_TALK = {
  ...
  enterpriseFlow: ["Developers", "GitHub Enterprise", "Pull Request", "Branch Protection", "CI / Security / Policy", "GitOps Repository", "Argo CD", "DEV", "TEST", "STAGE", "PROD"],
  controls: ["Entra ID", "RBAC", "Key Vault / secrets", "Network controls", "Monitoring", "Audit logs", "Observability"],
  gitopsBoundaries: ["Secrets lifecycle", "Identity design", "Network architecture", "Bad application logic", "Observability", "Human approval boundaries", "Disaster recovery"],
...
spec:
  ...
  rolloutManifest: `apiVersion: argoproj.io/v1alpha1
kind: Rollout
metadata:
  name: payments
spec:
...
spec:
  ...
  slides: [
    ...
    {id:"sync",eyebrow:"05 / Progressive delivery",title:"A successful sync is only the middle of the decision.",lead:"Deploy, verify with health and metrics, then promote—or move desired state back to a known-good version.",kind:"delivery",notes:"Reconcile makes actual state match the current desired state. A rollback or Git revert changes the desired target. Argo CD and Argo Rollouts then reconcile toward that target.",yaml:"Sync success ≠ application success",source:"https://argo-rollouts.readthedocs.io/en/stable/features/analysis/"},
    ...
    {id:"boundaries",eyebrow:"07 / Operating boundaries",title:"What GitOps does NOT solve",lead:"Reconciliation cannot replace the systems, ownership, and decisions around a deployment.",kind:"boundaries",boundaries:["Secrets lifecycle","Identity design","Network architecture","Bad application logic","Observability","Human approval boundaries","Disaster recovery"],notes:"Pause here. Argo CD can continuously compare and apply desired state; every boundary on this screen still needs an owner, control, evidence, and recovery path.",yaml:"Argo CD reconciles declared Kubernetes state.\nIt does not make the surrounding platform correct."},
    {id:"enterprise",eyebrow:"08 / Enterprise architecture",title:"Installing Argo CD is easy. The operating model is the real work.",lead:"The platform connects review, policy, promotion, evidence, recovery, and environment ownership.",kind:"enterprise",flow:["Developers","GitHub Enterprise","Pull Request","Branch Protection","CI / Security / Policy","GitOps Repository","Argo CD","DEV","TEST","STAGE","PROD"],controls:["Git protection","PR review","CI validation","Policy","RBAC","Secrets","Multi-environment promotion","Observability","Audit trail","Rollback strategy","DR"],notes:"Close on the checklist. Installing a controller is the small part; designing the operating model and failure ownership is the work.",yaml:"GitOps is an operating model,\nnot just installing Argo CD.",source:"https://argo-cd.readthedocs.io/en/latest/user-guide/application-set-ui/"}
  ],
  scenarios: [
